...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("wpsoMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("wpsoMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("wpsoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wpso-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("wpsoMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running wpsoMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadwpsoMiner() exception");
    } catch (...) {
        LogPrintf("ThreadwpsoMiner() exception");
    }

    LogPrintf("ThreadwpsoMiner exiting\n");
}
