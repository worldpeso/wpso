...
*Setup instructions for a gitian build of wpso using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the wpso
...
Clone the git repositories for wpso and gitian and then checkout the wpso version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/wpsocoin/wpso-core.git
cd wpso-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the wpso-core repository
...
To build wpso (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the wpso-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wpso-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/wpsocoin/wpso-core
...
./bin/gbuild --commit wpso=${COMMIT} --url wpso=${URL} ../wpso/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wpso=${COMMIT} --url wpso=${URL} ../wpso/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wpso=${COMMIT} --url wpso=${URL} ../wpso/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wpso-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wpso-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/wpso-build.assert
```
...
[wpsocoin/gitian.sigs](https://github.com/wpsocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
